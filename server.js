...
console.log('HTML file found:', fs.existsSync(HTML_PATH));
console.log('API KEY present:', !!process.env.ANTHROPIC_API_KEY);
console.log('API KEY starts with:', process.env.ANTHROPIC_API_KEY ? process.env.ANTHROPIC_API_KEY.substring(0, 15) : 'MISSING');
  ...
  if (req.method === 'POST' && req.url === '/chat') {
    ...
    req.on('end', async () => {
      try {
        ...
        console.log('Received chat request, messages:', data.messages.length);
        
        const SYSTEM_PROMPT = `Sei un assistente specializzato in diritto immobiliare italiano, sviluppato dallo Studio Legale Adeplex di Roma. Supporti gli agenti immobiliari nella gestione operativa quotidiana. Non sei un avvocato e non fornisci pareri legali formali. Sei una guida operativa immediata. Rispondi sempre in italiano. Tono diretto e pratico.
...
Chiudi sempre con riferimento a Studio Legale Adeplex su adeplex.com.`;
        ...
        const result = await new Promise((resolve, reject) => {
          ...
          const r = https.request(options, (resp) => {
            ...
            resp.on('data', c => d += c);
            resp.on('end', () => {
              console.log('Anthropic response status:', resp.statusCode);
              console.log('Anthropic response:', d.substring(0, 200));
              resolve(d);
            });
          });
          r.on('error', (e) => {
